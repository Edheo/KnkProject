...
namespace Vlc.DotNet.Forms
{
    public partial class VlcControl : Control, ISupportInitialize
    {
        ...
        private void VlcControlNeedLibDirectory()
        {
            ...
            if (Debugger.IsAttached)
            {
                if (AssemblyName.GetAssemblyName(currentAssembly.Location).ProcessorArchitecture == ProcessorArchitecture.X86)
                    VlcLibDirectory = new DirectoryInfo(Path.Combine(currentDirectory, @"..\..\..\VlcLib\x86\"));
                else
                    VlcLibDirectory = new DirectoryInfo(Path.Combine(currentDirectory, @"..\..\..\VlcLib\x64\"));
            }
            else
            {
                if (AssemblyName.GetAssemblyName(currentAssembly.Location).ProcessorArchitecture == ProcessorArchitecture.X86)
            ...
            if (!VlcLibDirectory.Exists)
            {
                ...
                {
                    VlcLibDirectory = new DirectoryInfo(folderBrowserDialog.SelectedPath);
                }
            }
        }
